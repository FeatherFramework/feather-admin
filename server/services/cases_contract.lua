...
RegisterCommand('AdminCasesPersistenceSmokeTest', function(source, args)
    ...
    local executed, committed = pcall(DB.transaction, function(tx)
        tx.exec([[INSERT INTO feather_admin_cases
        ...
        local row = (tx.query([[SELECT id, target_account_id, created_admin_account_id
        ...
        tx.exec([[UPDATE feather_admin_cases SET status = 'claimed', assigned_admin_account_id = ?,
        ...
        local claimed = (tx.query([[SELECT assigned_admin_account_id FROM feather_admin_cases
            WHERE id = ?]], row.id) or {})[1]
        verified.claimed = claimed and claimed.assigned_admin_account_id == identity.accountId

        tx.exec([[INSERT INTO feather_admin_case_links
        ...
        local linked = (tx.query([[SELECT admin_account_id FROM feather_admin_case_links
            WHERE case_id = ?]], row.id) or {})[1]
        verified.linked = linked and linked.admin_account_id == identity.accountId

        tx.exec([[UPDATE feather_admin_cases SET status = 'closed', resolution = 'smoke complete',
            ...
            identity.accountId, license, identity.accountName or identity.serverName,
              identity.characterId, identity.characterName, row.id)
        local closed = (tx.query([[SELECT closed_admin_account_id, status FROM feather_admin_cases
            WHERE id = ?]], row.id) or {})[1]
    ...
    local remainingCases = tonumber(DB.value(
        'SELECT COUNT(*) FROM feather_admin_cases WHERE title = ?', marker)) or -1
    local remainingLinks = tonumber(DB.value([[SELECT COUNT(*) FROM feather_admin_case_links l
