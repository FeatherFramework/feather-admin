...
local function target(accountId, characterId, serverId)
    ...
    if type(characterId) == 'string' and characterId ~= '' then
        ...
        row = DB.one([[SELECT id AS accountId, display_name AS targetName,
...
FeatherAdmin.RegisterRPC('feather-admin:notes:list', function(params, _, src)
    ...
    local rows = DB.query(([=[SELECT id, body, revision,
...
FeatherAdmin.RegisterRPC('feather-admin:notes:create', function(params, _, src)
    ...
    local executed, committed = pcall(DB.transaction, function(tx)
        noteId = tx.insert([[INSERT INTO feather_admin_player_notes
...
FeatherAdmin.RegisterRPC('feather-admin:notes:edit', function(params, _, src)
    ...
    local executed, committed = pcall(DB.transaction, function(tx)
        local row = tx.one([[SELECT id, target_account_id AS accountId, target_name AS targetName,
            target_character_id AS characterId, target_character_name AS characterName, revision
            FROM feather_admin_player_notes WHERE id = ? AND archived = 0 FOR UPDATE]], noteId)
        ...
        tx.exec([[UPDATE feather_admin_player_notes SET body = ?, revision = ?, updated_admin_account_id = ?,
...
FeatherAdmin.RegisterRPC('feather-admin:notes:archive', function(params, _, src)
    ...
    local row, verified
    local executed, committed = pcall(DB.transaction, function(tx)
        row = tx.one([[SELECT target_account_id AS accountId, target_name AS targetName,
        ...
        tx.exec([[UPDATE feather_admin_player_notes SET archived = 1, revision = ?,
...
FeatherAdmin.RegisterRPC('feather-admin:notes:history', function(params, _, src)
    ...
    local rows = DB.query(([=[SELECT revision, body, change_type AS changeType,
