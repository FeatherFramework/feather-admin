...
local function targetIdentity(row)
    return {
        ...
        license = row.license, name = row.playerName, characterId = tonumber(row.characterId),
...
local function adminIdentity(src)
    ...
    return { license = FeatherAdmin.Core.User.GetLicense(src),
        name = GetPlayerName(src) or ('Source %s'):format(src), characterId = tonumber(char.id),
...
local function activePlayers(src)
    ...
    for _, rawId in ipairs(GetPlayers()) do
        ...
        if char and playerId ~= src and FeatherAdmin.CanActOnLicense(src, license, 'staff.role.assign') == true then
            players[#players + 1] = { serverId = playerId, serverName = GetPlayerName(playerId),
                license = license, characterId = tonumber(char.id),
                characterName = ('%s %s'):format(char.first_name or '', char.last_name or ''):gsub('%s+$', ''),
                roleId = tonumber(char.role_id), roleName = char.role_name,
                roleLevel = tonumber(char.role_level) or 0, isOnline = true }
        end
    end
    return players
end
...
local function assignableRoles(src)
    ...
    return MySQL.query.await('SELECT id, name, level FROM roles WHERE level <= ? ORDER BY level ASC, name ASC',
        { actorLevel }) or {}
end
...
FeatherAdmin.RegisterRPC('feather-admin:staff:search', function(params, _, src)
    ...
    if roleId then clauses[#clauses + 1], values[#values + 1] = 'r.id = ?', roleId end
    ...
    ]=]):format(table.concat(clauses, ' AND '), pageSize + 1, offset), values) or {}
    ...
    local safeQuery = query:sub(1, 8):lower() == 'license:' and 'type=license' or query
    AdminAudit.Record(src, 'staff.search', nil,
        ('query=%s role_id=%s page=%s'):format(safeQuery, tostring(roleId or 'all'), page))
...
FeatherAdmin.RegisterRPC('feather-admin:staff:role:assign', function(params, _, src)
    ...
    local characterId, roleId, reason = tonumber(params.characterId), tonumber(params.roleId),
        trim(params.reason):gsub('[%c]', ' ')
    ...
    local changed = MySQL.update.await('UPDATE characters SET role_id = ? WHERE id = ? AND role_id = ?',
    ...
    if not historyId then
        print(('[feather-admin] Role change for character %s succeeded but its history row failed.'):format(characterId))
    end
