...
local function roleLabel(role)
    return ('%s (%s)'):format(tostring(role.name or role.roleName or AdminTranslate('not_available')),
        tostring(role.level or role.roleLevel or 0))
end
...
local function openTarget(target, origin)
    AdminStaff.selectedTarget, AdminStaff.origin, AdminStaff.reason = target, origin, ''
...
function AdminUI.OpenStaffManagement()
    ...
    local page, query = AdminUI.RegisterPage('staff_management'), AdminStaff.searchQuery or ''
    ...
    if #AdminStaff.players == 0 then
        ...
        for _, entry in ipairs(AdminStaff.players) do
            local target = entry
            AdminUI.AddHtmlButton(page, targetButton(target), function() openTarget(target, 'online') end)
        end
    end
...
function AdminUI.OpenStaffSearchResults()
    ...
    if #AdminStaff.results == 0 then
        ...
        for _, entry in ipairs(AdminStaff.results) do
            local target = entry
            AdminUI.AddHtmlButton(page, targetButton(target), function() openTarget(target, 'search') end)
        end
    end
    ...
    AdminUI.AddFooter(page)
    AdminUI.AddFooterButton(page, AdminTranslate('back'), function() AdminStaff.Request() end)
...
function AdminUI.OpenStaffRole()
    ...
    if #options == 0 then
        ...
        AdminUI.AddButton(page, AdminTranslate('continue'), function()
            ...
            local maxReason = math.min(tonumber(Config.staff.maxReasonLength) or 200, 200)
    ...
    AdminUI.AddFooterButton(page, AdminTranslate('back'), function()
        if AdminStaff.origin == 'search' then AdminUI.OpenStaffSearchResults() else AdminStaff.Request() end
    end)
